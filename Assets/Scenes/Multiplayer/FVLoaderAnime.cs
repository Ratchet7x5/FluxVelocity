...
namespace SDP21.DamienCo.FluxVelocity
{
	/// <summary>
	/// Simple behavior to animate particles around to create a typical "Ajax Loader". this is actually very important to visual inform the user that something is happening
	...
	/// 
	/// </summary>
	public class FVLoaderAnime : MonoBehaviour {
		...
		void Update () {
			...
			if (_isAnimating)
			{
				// we rotate over time. Time.deltaTime is mandatory to have a frame rate independent animation,
				...
				// so the particle is centered in the scene so that when it starts rotating, it doesn't jump and slowly we animate it to its final radius giving a smooth transition.
